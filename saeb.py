...
EXPR_FILTER_RESULTADOS = '[a["href"] for a in soup.find("div", id="parent-fieldname-text").find_all("a", class_="external-link") if str(self.year) in a["href"] and self.agg_finder in a["href"].lower() and "alfabe" not in a["href"]]'
...
class handleSaeb(handleDatabase):
            ...
            case 'uf':
                ...
                if self.year != 2017:
                    ...
                    with zipfile.ZipFile(self.filepaths[0], 'r') as zf:
                        fp = [fp for fp in zf.namelist() if 'TS_UF' in fp][0]
    ...
    def preprocess_df(self):
        ...
        match self.agg_level:
            case 'uf':
                self.df = self.preprocess_uf()
        return self.df
                
    def preprocess_uf(self):
        df = self.dfs
        if self.year < 2011:
            df.columns = COLUMNS_LABELS_UF[2011]
        elif self.year == 2015:
            df.columns = COLUMNS_LABELS_UF[2015]
    ...
    def otimize_df(self):
        ...
        match self.agg_level:
            case 'uf':
                self.df = self.otimize_uf()
        return self.df

    def otimize_uf(self):
        cols = ['CO_UF', 'NO_UF', 'DEPENDENCIA_ADM', 'LOCALIZACAO', 'CAPITAL']
        for col in cols:
